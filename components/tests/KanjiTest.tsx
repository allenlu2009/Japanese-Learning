...
import type { KanjiQuestion } from '@/lib/kanjiTestGenerator';
...
export function KanjiTest({
  ...
  useEffect(() => {
    if (currentQuestion) {
      ...
      // Play audio for the kanji if audio is enabled
      if (settings.enabled && !isAnswered) {
        playCharacterAudio(currentQuestion.kanji, {
          rate: settings.rate,
  ...
  return (
    <Card>
      <div className="space-y-6">
        ...
        {/* Kanji Display */}
        <div className="text-center py-12 bg-gradient-to-br from-blue-50 to-white rounded-lg border-2 border-blue-100">
          <div className="text-8xl md:text-9xl font-bold text-gray-900 mb-4 select-none">
            {currentQuestion.kanji}
          </div>
          <div className="text-gray-600 text-lg mb-2">
            Meaning: <span className="font-semibold text-blue-700">{currentQuestion.meanings.join(', ')}</span>
          </div>
          {currentQuestion.readingType !== 'mixed' && (
            <div className="text-sm text-gray-500 mt-2">
              Type the{' '}
              <span className="font-semibold text-blue-600">
                {currentQuestion.readingType === 'onyomi' ? 'onyomi (Chinese reading)' : 'kunyomi (Japanese reading)'}
              </span>
              {' '}in romanji
            </div>
          )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3">
            {!isAnswered ? (
              <Button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700" size="lg" disabled={!answer.trim()}>
                Submit Answer
              </Button>
            ) : (
              <Button
