...
export function VocabularyTest({
  ...
  useEffect(() => {
    if (currentQuestion) {
      ...
      // Play audio for the kana reading if audio is enabled
      // (Using kana instead of kanji for clearer pronunciation)
      ...
      <div className="space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3">
            {!isAnswered ? (
              <Button type="submit" className="flex-1 bg-orange-600 hover:bg-orange-700" size="lg" disabled={!answer.trim()}>
                Submit Answer
              </Button>
            ) : (
              <Button
